...
<div class="p-4 sm:p-6 space-y-4" x-data="leadScraperApp()" x-init="loadLeads()">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-4">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-2xl font-bold" x-text="leads.length"></p><p class="text-[11px] dark:text-white/40 text-gray-400">Total Leads</p></div>
            </div>
        </div>
        <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-4">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-2xl font-bold" x-text="leads.filter(l=>l.estado==='nuevo').length"></p><p class="text-[11px] dark:text-white/40 text-gray-400">Nuevos</p></div>
            </div>
        </div>
        <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-4">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-2xl font-bold" x-text="leads.filter(l=>l.estado==='contactado').length"></p><p class="text-[11px] dark:text-white/40 text-gray-400">Contactados</p></div>
            </div>
        </div>
        <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-4">
            <div class="flex items-center gap-3">
                ...
                <div><p class="text-2xl font-bold" x-text="leads.filter(l=>l.estado==='convertido').length"></p><p class="text-[11px] dark:text-white/40 text-gray-400">Convertidos</p></div>
            </div>
        </div>
    </div>
    ...
    <div class="flex gap-1 p-1 dark:bg-dark-900/60 bg-gray-100 rounded-2xl w-fit">
        <button @click="activeTab='buscar'" class="px-5 py-2 rounded-xl text-sm font-medium transition-all" :class="activeTab==='buscar' ? 'bg-nexo-500 text-white shadow-lg shadow-nexo-500/25' : 'dark:text-white/50 text-gray-500 dark:hover:text-white/80 hover:text-gray-700'">
            <span class="flex items-center gap-2"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg> Buscar Leads</span>
        </button>
        <button @click="activeTab='leads'" class="px-5 py-2 rounded-xl text-sm font-medium transition-all" :class="activeTab==='leads' ? 'bg-nexo-500 text-white shadow-lg shadow-nexo-500/25' : 'dark:text-white/50 text-gray-500 dark:hover:text-white/80 hover:text-gray-700'">
            <span class="flex items-center gap-2"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg> Mis Leads <span class="ml-1 text-xs opacity-70" x-text="'('+leads.length+')'"></span></span>
        </button>
    </div>
    ...
    <div x-show="activeTab === 'buscar'" x-cloak class="space-y-4">
        ...
        <div class="dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-5">
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="sm:w-56">
                    <label class="text-[10px] font-semibold dark:text-white/40 text-gray-400 uppercase tracking-wider mb-1 block">Nicho</label>
                ...
                <div class="flex-1">
                    <label class="text-[10px] font-semibold dark:text-white/40 text-gray-400 uppercase tracking-wider mb-1 block">Ubicación</label>
                    <input type="text" x-model="searchLocation" placeholder="Ciudad, País... (ej: Madrid, España)" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50" @keydown.enter="doSearch()">
                </div>
        ...
        <div x-show="searchResults.length > 0">
            ...
            <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
                <div class="flex items-center gap-3">
                    <button @click="toggleSelectAll()" class="flex items-center gap-2.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all cursor-pointer select-none" :class="allSelected ? 'dark:bg-nexo-500/15 bg-nexo-50 text-nexo-400 border border-nexo-500/30' : 'dark:bg-white/5 bg-gray-100 dark:text-white/50 text-gray-500 border dark:border-white/[0.06] border-gray-200 hover:border-nexo-500/30'">
                        ...
                        <span x-text="allSelected ? 'Deseleccionar' : 'Seleccionar todos'"></span>
                    </button>
                    <p class="text-sm dark:text-white/50 text-gray-500"><span x-text="visibleResults.length" class="font-bold text-nexo-400"></span> resultados para &ldquo;<span x-text="lastQuery" class="italic"></span>&rdquo;</p>
                </div>
                <div class="flex items-center gap-2">
                    <span x-show="selectedCount > 0" class="text-xs font-medium px-2.5 py-1 rounded-lg bg-nexo-500/10 text-nexo-400" x-text="selectedCount + ' seleccionado' + (selectedCount > 1 ? 's' : '')"></span>
                    <span x-show="searchPage > 0" class="text-[10px] dark:text-white/30 text-gray-400">Página <span x-text="searchPage + 1"></span></span>
                </div>
            </div>
            ...
            <div x-show="selectedCount > 0" x-transition class="mb-4 p-3 dark:bg-nexo-500/10 bg-purple-50 rounded-2xl border dark:border-nexo-500/20 border-purple-200 flex flex-col sm:flex-row items-start sm:items-center gap-3">
                <div class="flex items-center gap-2 flex-1">
                    ...
                    <span class="text-sm font-medium dark:text-nexo-300 text-nexo-600" x-text="selectedCount + ' lead' + (selectedCount > 1 ? 's' : '') + ' seleccionado' + (selectedCount > 1 ? 's' : '')"></span>
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <button @click="bulkAssign()" :disabled="!bulkAssignUserId || bulkAssigning" class="px-4 py-2 rounded-lg text-xs font-bold text-white bg-nexo-500 hover:bg-nexo-600 transition-colors disabled:opacity-50 flex items-center gap-1.5">
                        ...
                        <span x-text="bulkAssigning ? 'Asignando...' : 'Asignar seleccionados'"></span>
                    </button>
                    <button @click="bulkDismiss()" class="px-3 py-2 rounded-lg text-xs font-medium dark:text-white/40 text-gray-500 dark:hover:bg-white/5 hover:bg-gray-100 transition-colors">Descartar</button>
                </div>
            </div>
                ...
                <template x-for="(sr, idx) in searchResults" :key="idx">
                    <div x-show="!sr.dismissed" class="dark:bg-dark-800 bg-white rounded-2xl border transition-all relative" :class="sr.selected ? 'dark:border-nexo-500/40 border-nexo-400 shadow-lg shadow-nexo-500/10' : 'dark:border-white/[0.06] border-gray-200 hover:shadow-lg hover:shadow-nexo-500/5'">
                        ...
                        <div x-show="sr.isDuplicate" class="mx-4 mt-2 mb-0 px-2.5 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20 flex items-center gap-2">
                            ...
                            <span class="text-[10px] text-amber-400 font-medium">Ya existe como cliente: <span x-text="sr.duplicateInfo" class="font-bold"></span></span>
                        </div>
                        <!-- Assigned badge -->
                        <div x-show="sr.assigned" class="mx-4 mt-2 mb-0 px-2.5 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-2">
                            ...
                            <span class="text-[10px] text-emerald-400 font-medium">Asignado a <span x-text="sr.assignedTo" class="font-bold"></span></span>
                        </div>
                        <div class="p-4 pt-2">
                            ...
                            <div x-show="sr.enriched" class="mb-3 p-2.5 rounded-lg dark:bg-white/[0.03] bg-gray-50 border dark:border-white/[0.04] border-gray-100 space-y-2">
                                <p class="text-[10px] font-semibold dark:text-white/40 text-gray-400 uppercase mb-1">Datos del sitio web</p>
                                ...
                                <template x-for="(ph, pi) in (sr.allPhones||[])" :key="'p'+pi">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span x-show="pi===0" class="text-[9px] px-1.5 py-0.5 rounded dark:bg-emerald-500/10 bg-emerald-50 text-emerald-400">Principal</span>
                                    </div>
                                </template>
                            ...
                            <div class="flex gap-1.5 pt-3 border-t dark:border-white/[0.06] border-gray-100">
                                <button @click="enrichResult(sr)" :disabled="sr.enriching||sr.enriched" class="flex-1 px-2 py-1.5 rounded-lg text-[10px] font-medium transition-colors flex items-center justify-center gap-1" :class="sr.enriched?'bg-emerald-500/10 text-emerald-400':sr.enriching?'bg-amber-500/10 text-amber-400':'dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200'">
                                    ...
                                    <span x-text="sr.enriched?'Escaneado':sr.enriching?'Escaneando...':'Escanear Web'"></span>
                                </button>
                                <button @click="openAssignFromSearch(sr)" class="flex-1 px-2 py-1.5 rounded-lg text-[10px] font-medium bg-nexo-500/10 text-nexo-400 hover:bg-nexo-500/20 transition-colors flex items-center justify-center gap-1" :class="sr.assigned&&'opacity-50 cursor-not-allowed'" :disabled="sr.assigned">
                                    ...
                                    <span x-text="sr.assigned?'Asignado':'Asignar'"></span>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex justify-center mt-6">
                <button @click="loadMore()" :disabled="loadingMore" class="px-6 py-3 rounded-xl text-sm font-medium dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200 transition-colors flex items-center gap-2 border dark:border-white/[0.06] border-gray-200">
                    ...
                    <span x-text="loadingMore ? 'Cargando más...' : 'Cargar +10 resultados'"></span>
                </button>
            </div>
        </div>
        ...
        <div x-show="searchResults.length===0 && !searching && !searchDone" class="text-center py-16">
            ...
            <h3 class="font-bold mb-1 dark:text-white/60 text-gray-600">Busca leads reales en la web</h3>
            <p class="text-sm dark:text-white/30 text-gray-400 max-w-md mx-auto">Selecciona un nicho y ubicación para encontrar empresas con datos de contacto reales. Escanea sus sitios web para extraer emails, teléfonos y redes sociales.</p>
        </div>
        <div x-show="searchResults.length===0 && searchDone && !searching" class="text-center py-16">
            <h3 class="font-bold mb-1 dark:text-white/60 text-gray-600">No se encontraron resultados</h3>
            <p class="text-sm dark:text-white/30 text-gray-400">Prueba con otro nicho o ubicación.</p>
        </div>
    </div>
    ...
    <div class="dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-4">
        <div class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" x-model="search" placeholder="Buscar por empresa, contacto, email, teléfono..." class="w-full pl-10 pr-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50">
            </div>
        ...
        <template x-for="lead in filteredLeads" :key="lead.id">
            <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 overflow-hidden hover:shadow-lg hover:shadow-nexo-500/5 transition-all group" :class="lead.estado === 'descartado' ? 'opacity-50' : ''">
                ...
                <div class="p-4">
                    ...
                    <p x-show="lead.nicho" class="text-[10px] dark:text-white/30 text-gray-400 mb-2" x-text="'Nicho: ' + lead.nicho"></p>
                    ...
                    <div x-show="lead.asignado_nombre" class="mb-3 px-2.5 py-1.5 rounded-lg dark:bg-white/[0.03] bg-gray-50 border dark:border-white/[0.04] border-gray-100">
                        <span class="text-[10px] dark:text-white/30 text-gray-400">Asignado a: </span>
                    ...
                    <div class="flex gap-1.5 pt-3 border-t dark:border-white/[0.06] border-gray-100">
                        ...
                        <button @click="openAssign(lead)" class="flex-1 px-2 py-1.5 rounded-lg text-[10px] font-medium bg-nexo-500/10 text-nexo-400 hover:bg-nexo-500/20 transition-colors flex items-center justify-center gap-1" title="Asignar a funcionario">
                        ...
                        <button @click="deleteLead(lead)" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-red-500/10 transition-colors shrink-0" title="Eliminar">
    ...
    <div x-show="filteredLeads.length === 0" class="text-center py-20">
        ...
        <h3 class="font-bold mb-1 dark:text-white/60 text-gray-600" x-text="leads.length === 0 ? 'Aún no tienes leads' : 'Sin resultados'"></h3>
        <p class="text-sm dark:text-white/30 text-gray-400 max-w-md mx-auto" x-text="leads.length === 0 ? 'Crea tu primer lead manualmente o importa un CSV con tu lista de prospectos.' : 'Prueba con otro filtro o término de búsqueda.'"></p>
            ...
            <div class="relative w-full max-w-lg dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.08] border-gray-200 shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-5">
                        <h3 class="text-lg font-bold" x-text="editingLead ? 'Editar Lead' : 'Nuevo Lead'"></h3>
                    ...
                    <form @submit.prevent="saveLead()" class="space-y-4">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Sitio Web</label><input type="url" x-model="leadForm.sitio_web" placeholder="https://..." class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50"></div>
                            </div>
                            <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Dirección</label><input type="text" x-model="leadForm.direccion" placeholder="Ciudad, País" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50"></div>
                        </div>
                        <!-- Contact info -->
                        <div class="space-y-3">
                            <p class="text-xs font-semibold dark:text-white/50 text-gray-400 uppercase tracking-wider">Contacto</p>
                            <div class="grid grid-cols-2 gap-3">
                                <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Nombre Contacto</label><input type="text" x-model="leadForm.nombre_contacto" placeholder="Juan Pérez" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50"></div>
                                <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Cargo</label><input type="text" x-model="leadForm.cargo" placeholder="CEO, Director..." class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50"></div>
                                <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Email</label><input type="email" x-model="leadForm.email" placeholder="contacto@empresa.com" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50"></div>
                                <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Teléfono</label><input type="text" x-model="leadForm.telefono" placeholder="+55 11 9999-9999" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50"></div>
                                <div class="col-span-2"><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">WhatsApp</label><input type="text" x-model="leadForm.whatsapp" placeholder="+55 11 99999-9999" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50"></div>
                            </div>
                        </div>
                        <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Descripción / Notas</label><textarea x-model="leadForm.descripcion" rows="2" placeholder="¿A qué se dedica? ¿Cómo lo encontraste?" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50 resize-none"></textarea></div>
                        <div class="flex gap-3 pt-2">
                            <button type="button" @click="showLeadModal = false" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200 transition-colors">Cancelar</button>
                            <button type="submit" class="flex-1 btn-purple px-4 py-2.5 rounded-xl text-sm font-medium text-white">
                                <span x-text="editingLead ? 'Guardar Cambios' : 'Crear Lead'"></span>
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <div class="relative w-full max-w-md dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.08] border-gray-200 shadow-2xl overflow-hidden">
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-bold">Importar Leads desde CSV</h3>
                    ...
                    <div class="dark:bg-white/[0.03] bg-gray-50 rounded-xl border dark:border-white/[0.06] border-gray-200 p-4 mb-4">
                        <p class="text-xs dark:text-white/50 text-gray-500 font-medium mb-2">Columnas aceptadas en el CSV:</p>
                        ...
                        <p class="text-[10px] dark:text-white/30 text-gray-400 mt-2">También acepta: empresa, company, contacto, phone, email, website, address, etc.</p>
                    </div>

                    <form @submit.prevent="importCSV()" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Nicho por defecto (opcional)</label>
                        ...
                        <div class="mb-4">
                            <label class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed dark:border-white/10 border-gray-300 rounded-xl cursor-pointer dark:hover:border-nexo-500/30 hover:border-nexo-500/50 transition-colors"
                                ...
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-xs dark:text-white/40 text-gray-400" x-text="csvFile ? csvFile.name : 'Haz click o arrastra tu archivo CSV aquí'"></p>
                                </div>
                                <input type="file" accept=".csv,.txt" class="hidden" @change="csvFile = $event.target.files[0]">
                            </label>
                        </div>
                        <div class="flex gap-3">
                            <button type="button" @click="showImportModal = false" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200 transition-colors">Cancelar</button>
    ...
    <template x-if="showAssignModal">
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="showAssignModal = false">
            ...
            <div class="relative w-full max-w-sm dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.08] border-gray-200 shadow-2xl overflow-hidden">
                <div class="h-1 bg-nexo-500"></div>
                <div class="p-6">
                    <h3 class="text-lg font-bold mb-1">Asignar Lead</h3>
                    <p class="text-xs dark:text-white/40 text-gray-400 mb-4" x-text="'Asignar ' + (assignTarget?.nombre_empresa || '') + ' a un funcionario'"></p>
                    ...
                    <div class="flex gap-3">
                        <button @click="showAssignModal = false" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200 transition-colors">Cancelar</button>
                        <button @click="doAssign()" :disabled="!assignUserId" class="flex-1 btn-purple px-4 py-2.5 rounded-xl text-sm font-medium text-white disabled:opacity-50">Asignar</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-show="showSearchAssignModal" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-[9999] flex items-center justify-center p-4" style="position:fixed;top:0;left:0;right:0;bottom:0;" @keydown.escape.window="showSearchAssignModal = false">
        ...
        <div class="relative w-full max-w-sm dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.08] border-gray-200 shadow-2xl overflow-hidden" @click.stop>
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h3 class="text-lg font-bold">Asignar como Cliente</h3>
                ...
                <div class="space-y-3 mb-5">
                    <div>
                        <label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Funcionario *</label>
                    ...
                    <div>
                        <label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Etapa</label>
                ...
                <div class="flex gap-3">
                    <button @click="showSearchAssignModal = false" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200 transition-colors">Cancelar</button>
                    <button @click="doAssignFromSearch()" :disabled="!searchAssignUserId" class="flex-1 btn-purple px-4 py-2.5 rounded-xl text-sm font-medium text-white disabled:opacity-50">Asignar</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <template x-if="showPipelineModal">
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="showPipelineModal = false">
            ...
            <div class="relative w-full max-w-sm dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.08] border-gray-200 shadow-2xl overflow-hidden">
                ...
                <div class="p-6">
                    <h3 class="text-lg font-bold mb-1">Enviar al Pipeline</h3>
                    <p class="text-xs dark:text-white/40 text-gray-400 mb-4" x-text="pipelineTarget?.nombre_empresa || ''"></p>
                    <div class="space-y-3 mb-4">
                        <div>
                            <label class="text-xs dark:text-white/50 text-gray-500 mb-1 block">Asignar a</label>
                    ...
                    <div class="flex gap-3">
                        <button @click="showPipelineModal = false" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200 transition-colors">Cancelar</button>
                        <button @click="doPipeline()" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium text-white bg-emerald-500 hover:bg-emerald-600 transition-colors">Enviar</button>
                    </div>
                </div>
            </div>
        </div>
...
function leadScraperApp() {
    return {
        ...
        async saveLead() {
            ...
            try {
                ...
                if (d.ok) {
                    ...
                    this.toast = this.editingLead ? 'Lead actualizado' : 'Lead creado';
                    this.toastType = 'ok';
        ...
        async importCSV() {
            ...
            try {
                ...
                if (d.ok) {
                    ...
                    this.csvFile = null;
                    this.toast = `${d.inserted} leads importados` + (d.skipped ? `, ${d.skipped} omitidos` : '');
                    this.toastType = 'ok';
                    await this.loadLeads();
                } else {
                    this.toast = d.error || 'Error en la importación'; this.toastType = 'err';
                }
            } catch(e) { this.toast = 'Error de conexión'; this.toastType = 'err'; }
        ...
        async doPipeline() {
            ...
            try {
                ...
                if (d.ok) {
                    ...
                    this.toast = 'Lead enviado al pipeline como cliente';
                    this.toastType = 'ok';
                    await this.loadLeads();
                } else {
                    this.toast = d.error || 'Error'; this.toastType = 'err';
        ...
        async doSearch() {
            ...
            try {
                ...
                if (d.ok) {
                    ...
                    this.toast = d.error || 'Error en la búsqueda'; this.toastType = 'err';
                }
            } catch(e) { this.toast = 'Error de conexión'; this.toastType = 'err'; }
        ...
        async loadMore() {
            ...
            try {
                ...
                if (d.ok && d.results.length > 0) {
                    ...
                    this.toast = newResults.length + ' resultados más cargados'; this.toastType = 'ok';
                } else {
                    this.toast = 'No hay más resultados'; this.toastType = 'err';
                    this.searchPage--;
                }
        ...
        async enrichResult(sr) {
            ...
            try {
                ...
                if (d.ok) {
                    ...
                    this.toast = d.error || 'No se pudo escanear'; this.toastType = 'err';
                }
            } catch(e) { this.toast = 'Error al escanear'; this.toastType = 'err'; }
            sr.enriching = false;
        },
        ...
        async doAssignFromSearch() {
            ...
            try {
                ...
                if (d.ok) {
                    ...
                    this.toast = 'Cliente creado y asignado'; this.toastType = 'ok';
                } else {
                    this.toast = d.error || 'Error'; this.toastType = 'err';
                }
            } catch(e) { this.toast = 'Error de conexión'; this.toastType = 'err'; }
        ...
        async bulkAssign() {
            ...
            if (!selected.length) { this.toast = 'Selecciona al menos un lead'; this.toastType = 'err'; return; }
            ...
            try {
                ...
                if (d.ok) {
                    ...
                    this.toast = d.created + ' clientes creados y asignados'; this.toastType = 'ok';
                } else {
                    this.toast = d.error || 'Error'; this.toastType = 'err';
                }
            } catch(e) { this.toast = 'Error de conexión'; this.toastType = 'err'; }
