...
<div class="p-4 sm:p-6 space-y-4" x-data="usersApp()">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-4">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs dark:text-white/40 text-gray-400"><?php echo __('usr_total'); ?></span>
        ...
        <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-4">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs dark:text-white/40 text-gray-400"><?php echo __('usr_activos'); ?></span>
        ...
        <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-4">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs dark:text-white/40 text-gray-400"><?php echo __('usr_admin'); ?></span>
        ...
        <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-4">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs dark:text-white/40 text-gray-400"><?php echo __('usr_en_linea'); ?></span>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div class="relative flex-1 w-full sm:max-w-xs">
            ...
            <input type="text" x-model="search" placeholder="<?php echo __('usr_buscar'); ?>" class="w-full pl-9 pr-3 py-2 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50">
        </div>
        <div class="flex items-center gap-2 flex-wrap">
            <select x-model="filterRol" class="px-3 py-2 text-xs rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none">
                <option value=""><?php echo __('usr_todos_roles'); ?></option>
            ...
            <select x-model="filterStatus" class="px-3 py-2 text-xs rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none">
                ...
                <option value="1"><?php echo __('filtro_activos'); ?></option>
                <option value="0"><?php echo __('filtro_inactivos'); ?></option>
            </select>
            ...
            <button @click="showModal = true" class="btn-purple px-4 py-2 rounded-xl text-xs font-medium text-white flex items-center gap-1.5">
                ...
                <?php echo __('usr_nuevo_btn'); ?>
            </button>
        </div>
    </div>

    <p class="text-xs dark:text-white/30 text-gray-400" x-text="filtered().length + ' ' + _t.de + ' ' + users.length + ' ' + _t.usuarios_pl.toLowerCase()"></p>
            ...
            <div class="stat-card dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 p-5 transition-all hover:shadow-lg hover:dark:border-white/[0.12] hover:border-gray-300">
                ...
                <div class="grid grid-cols-3 gap-2 mt-3 pt-3 border-t dark:border-white/[0.06] border-gray-100">
                    <div class="text-center">
                        ...
                        <p class="text-[9px] dark:text-white/30 text-gray-400"><?php echo __('nav_clientes'); ?></p>
                    </div>
                    <div class="text-center">
                        <p class="text-sm font-bold" x-text="u.total_facturas"></p>
                        <p class="text-[9px] dark:text-white/30 text-gray-400"><?php echo __('nav_facturas'); ?></p>
                    </div>
                    <div class="text-center">
                        <p class="text-sm font-bold dark:text-emerald-400 text-emerald-600" x-text="'$' + Number(u.ingresos_generados).toLocaleString()"></p>
                        <p class="text-[9px] dark:text-white/30 text-gray-400"><?php echo __('tabla_ingresos'); ?></p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-1.5 mt-2">
                    ...
                    <span class="text-[10px] dark:text-white/25 text-gray-300" x-text="isOnline(u.ultimo_acceso) ? _t.en_linea : (u.ultimo_acceso ? timeAgo(u.ultimo_acceso) : _t.nunca)"></span>
                </div>

                <!-- Actions -->
                <div class="flex gap-2 mt-3">
                    <a :href="'perfil.php?id='+u.id" class="flex-1 px-3 py-1.5 text-xs rounded-lg dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200 text-center transition-colors font-medium" x-text="_t.ver"></a>
                    <button @click="openEdit(u)" class="flex-1 px-3 py-1.5 text-xs rounded-lg bg-nexo-600/10 text-nexo-400 hover:bg-nexo-600/20 transition-colors font-medium" x-text="_t.editar"></button>
    ...
    <div x-show="viewMode === 'table'" class="dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/[0.06] border-gray-200 overflow-hidden">
        <table class="w-full text-sm">
            <thead><tr class="border-b dark:border-white/[0.06] border-gray-100">
                <th class="text-left px-4 py-3 text-xs font-medium dark:text-white/40 text-gray-400"><?php echo __('tabla_usuario'); ?></th>
                <th class="text-left px-4 py-3 text-xs font-medium dark:text-white/40 text-gray-400"><?php echo __('tabla_rol'); ?></th>
                <th class="text-center px-4 py-3 text-xs font-medium dark:text-white/40 text-gray-400"><?php echo __('nav_clientes'); ?></th>
                <th class="text-center px-4 py-3 text-xs font-medium dark:text-white/40 text-gray-400"><?php echo __('nav_facturas'); ?></th>
                <th class="text-right px-4 py-3 text-xs font-medium dark:text-white/40 text-gray-400"><?php echo __('tabla_ingresos'); ?></th>
                <th class="text-center px-4 py-3 text-xs font-medium dark:text-white/40 text-gray-400"><?php echo __('tabla_estado'); ?></th>
                <th class="text-left px-4 py-3 text-xs font-medium dark:text-white/40 text-gray-400"><?php echo __('usr_ultimo_acceso'); ?></th>
            ...
            <tr class="border-b dark:border-white/[0.04] border-gray-50 hover:dark:bg-white/[0.02] hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3 text-center">
                    <span class="text-[10px] font-medium px-2 py-0.5 rounded-full" :class="u.activo ? 'bg-emerald-500/15 text-emerald-400' : 'bg-red-500/15 text-red-400'" x-text="u.activo ? _t.activo : _t.inactivo"></span>
                </td>
                <td class="px-4 py-3">
                    <div class="flex items-center gap-1.5">
                        <div class="w-1.5 h-1.5 rounded-full" :class="isOnline(u.ultimo_acceso) ? 'bg-emerald-400' : 'bg-gray-500'"></div>
                        <span class="text-xs dark:text-white/40 text-gray-400" x-text="isOnline(u.ultimo_acceso) ? _t.en_linea : (u.ultimo_acceso ? timeAgo(u.ultimo_acceso) : _t.nunca)"></span>
                    </div>
                </td>
                <td class="px-4 py-3 text-center">
    ...
    <div x-show="showModal" x-transition x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/10 border-gray-200 shadow-2xl p-6">
            <h3 class="text-lg font-bold mb-4"><?php echo __('usr_nuevo'); ?></h3>
            <form method="POST" action="api/usuarios.php" class="space-y-3">
                <input type="hidden" name="action" value="create">
                <div class="grid grid-cols-2 gap-3">
                    <div class="col-span-2"><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block"><?php echo __('usr_nombre'); ?> *</label>
                    ...
                    <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block"><?php echo __('usr_email'); ?> *</label>
                    ...
                    <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block"><?php echo __('usr_contrasena'); ?> *</label>
                    ...
                    <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block"><?php echo __('usr_rol'); ?></label>
                        <select name="rol" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none">
                            <option value="vendedor"><?php echo __('usr_vendedor'); ?></option><option value="gerente"><?php echo __('usr_gerente'); ?></option><option value="admin"><?php echo __('usr_admin'); ?></option><option value="soporte"><?php echo __('usr_soporte'); ?></option></select></div>
                </div>
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="flex-1 btn-purple px-4 py-2.5 rounded-xl text-sm font-medium text-white"><?php echo __('usr_crear_usuario'); ?></button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="showEdit" x-transition x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/10 border-gray-200 shadow-2xl p-6">
            <h3 class="text-lg font-bold mb-4"><?php echo __('usr_editar'); ?></h3>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block"><?php echo __('usr_rol'); ?></label>
                        <select x-model="editUser.rol" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none">
                            <option value="vendedor"><?php echo __('usr_vendedor'); ?></option><option value="gerente"><?php echo __('usr_gerente'); ?></option><option value="admin"><?php echo __('usr_admin'); ?></option><option value="soporte"><?php echo __('usr_soporte'); ?></option></select></div>
                    <div class="col-span-2"><label class="text-xs dark:text-white/50 text-gray-500 mb-1 block"><?php echo __('usr_nueva_contrasena'); ?></label>
                        <input type="password" x-model="editUser.password" placeholder="<?php echo __('usr_dejar_vacio'); ?>" minlength="6" class="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-white/5 bg-gray-50 border dark:border-white/10 border-gray-200 outline-none focus:border-nexo-500/50"></div>
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="button" @click="showEdit = false" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200 transition-colors"><?php echo __('btn_cancelar'); ?></button>
                    <button type="submit" class="flex-1 btn-purple px-4 py-2.5 rounded-xl text-sm font-medium text-white" :disabled="saving" x-text="saving ? '<?php echo __('usr_guardando'); ?>' : '<?php echo __('btn_guardar'); ?>'"></button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="showConfirm" x-transition x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-sm dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/10 border-gray-200 shadow-2xl p-6 text-center">
            ...
            <h3 class="font-bold mb-1" x-text="confirmAction.activo ? '<?php echo __('usr_desactivar_preg'); ?>' : '<?php echo __('usr_activar_preg'); ?>'"></h3>
            <p class="text-sm dark:text-white/50 text-gray-500 mb-4" x-text="confirmAction.nombre + ' (' + confirmAction.email + ')'"></p>
            <div class="flex gap-3">
                <button @click="showConfirm = false" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium dark:bg-white/5 bg-gray-100 dark:hover:bg-white/10 hover:bg-gray-200 transition-colors"><?php echo __('btn_cancelar'); ?></button>
                <button @click="doToggle()" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium text-white transition-colors" :class="confirmAction.activo ? 'bg-red-600 hover:bg-red-700' : 'bg-emerald-600 hover:bg-emerald-700'" x-text="confirmAction.activo ? '<?php echo __('btn_desactivar'); ?>' : '<?php echo __('btn_activar'); ?>'"></button>
            </div>
        </div>
    </div>
    ...
    <div x-show="showDeleteConfirm" x-transition x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-sm dark:bg-dark-800 bg-white rounded-2xl border dark:border-white/10 border-gray-200 shadow-2xl p-6 text-center">
            ...
            <h3 class="font-bold mb-1"><?php echo __('usr_eliminar_perm_titulo'); ?></h3>
            <p class="text-sm dark:text-white/50 text-gray-500 mb-1" x-text="deleteTarget.nombre + ' (' + deleteTarget.email + ')'"></p>
            <p class="text-xs text-red-400 mb-4"><?php echo __('usr_eliminar_aviso'); ?></p>
            <div class="flex gap-3">
                ...
                <button @click="doDelete()" class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium text-white bg-red-600 hover:bg-red-700 transition-colors"><?php echo __('btn_eliminar'); ?></button>
            </div>
        </div>
    </div>
</div>
...
<script>
const _t = {
    en_linea: <?php echo json_encode(__('usr_en_linea')); ?>,
    nunca: <?php echo json_encode(__('time_nunca', 'Nunca')); ?>,
    ver: <?php echo json_encode(__('btn_ver')); ?>,
    editar: <?php echo json_encode(__('btn_editar')); ?>,
    activo: <?php echo json_encode(__('usr_activo')); ?>,
    inactivo: <?php echo json_encode(__('usr_inactivo')); ?>,
    usuario: <?php echo json_encode(__('tabla_usuario')); ?>,
    rol: <?php echo json_encode(__('tabla_rol')); ?>,
    clientes: <?php echo json_encode(__('nav_clientes')); ?>,
    facturas: <?php echo json_encode(__('nav_facturas')); ?>,
    ingresos: <?php echo json_encode(__('tabla_ingresos')); ?>,
    estado: <?php echo json_encode(__('tabla_estado')); ?>,
    ultimo_acceso: <?php echo json_encode(__('usr_ultimo_acceso')); ?>,
    de: <?php echo json_encode(__('paginacion_de')); ?>,
    usuarios_pl: <?php echo json_encode(__('usr_titulo')); ?>,
};
function usersApp() {
    return {
        ...
        exportCSV() {
            const rows = [[_t.usuario, 'Email', _t.rol, _t.clientes, _t.facturas, _t.ingresos, _t.estado, _t.ultimo_acceso]];
            this.filtered().forEach(u => {
                rows.push([u.nombre, u.email, u.rol, u.total_clientes, u.total_facturas, u.ingresos_generados, u.activo ? _t.activo : _t.inactivo, u.ultimo_acceso || _t.nunca]);
            });
